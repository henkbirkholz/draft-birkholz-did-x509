...
A did:x509 identifier string encapsulates elements extracted from an X.509 certificate chain, including the CA fingerprint and policy predicates that reference certificate fields such as subject names, subject alternative names, and extended key usage. While it is acceptable to use a did:x509 identifier as an opaque handle when it has been endorsed through an external trust mechanism, such as a verifiable credential or a trusted registry, implementers MUST NOT parse or interpret individual components of the identifier string for authorization decisions unless the identifier has been resolved against a verified certificate chain.
